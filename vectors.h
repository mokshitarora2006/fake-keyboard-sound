...
#define _get_header(s) ((_metadata *)(*s) - 1)
#define vector_push_back(arr, v)                                               \
  do {                                                                         \
    if (_is_full(arr)) {                                                       \
      _increase_size_push_back((void **)&(arr));                               \
    }                                                                          \
    (arr)[vector_size(arr)] = (v);                                             \
    _metadata *header = (_metadata *)((char *)(arr) - sizeof(_metadata));      \
    header->size++;                                                            \
  } while (0)
#define _is_full(s) ((_header(s)->size == _header(s)->capacity ? 1 : 0))
#define _is_empty(s) ((_header(s)->size) == 0)
#define vector_print(s, id)                                                    \
  do {                                                                         \
    _metadata *header = _header(s);                                            \
    if (header->size == 0) {                                                   \
      printf("{}\n");                                                          \
    } else {                                                                   \
      printf("{");                                                             \
...
#define vector_size(s) ((_header(s))->size)
#define vector_capacity(s) ((_header(s))->capacity)
#define vector_free(s) (_free_vector((void **)&s))
...
static inline void *_vector_create(size_t el_size) {
  size_t initial_size = 10;
  _metadata *header =
      (_metadata *)malloc(sizeof(_metadata) + initial_size * el_size);
  if (header == NULL) {
...
static inline void *vector_create_with_pre_allocation(size_t el_size,
                                                      size_t num) {
...
static inline void _increase_size_push_back(void **arr) {
  _metadata *header = (_metadata *)(*arr) - 1;
  if (header->capacity == header->size) {
    header->capacity *= 2;
    header = (_metadata *)realloc(
        header, header->capacity * header->element_size + sizeof(_metadata));
    if (header == NULL) {
      printf("Issues in reallocation of storage\n");
      exit(1);
    }
...
static inline void _free_vector(void **arr) {
  _metadata *header = (_metadata *)(*arr) - 1;
  free(header);
}

static inline void _reuse_vector(void **arr) {
  _metadata *header = (_metadata *)(*arr) - 1;
  header->size = 0;
}
